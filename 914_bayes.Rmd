...
# -----------------------
# Helper: Convergence (R-hat) and ESS
# -----------------------
diagnostic_stats <- function(fit, model_label, roi_label) {
  
  # Extract summary which contains Rhat, Bulk_ESS, and Tail_ESS
  fit_summ <- summary(fit)
  
  # Get max R-hat across all model parameters (fixed and random)
  all_rhats <- brms::rhat(fit)
  max_rhat <- max(all_rhats, na.rm = TRUE)
  
  # Get minimum ESS for the fixed effects
  min_bulk_ess <- min(fit_summ$fixed$Bulk_ESS, na.rm = TRUE)
  min_tail_ess <- min(fit_summ$fixed$Tail_ESS, na.rm = TRUE)
...
all_loo_stats  <- list()
all_emm        <- list()
all_diag_stats <- list() # <--- NEW: Initialize the empty list here
...
for (i in seq_len(nrow(fits_index))) {
  ...
  # <--- NEW: Run the diagnostic function and save it to the list
  all_diag_stats[[key]] <- diagnostic_stats(fit, model_label, roi_label) 
}
...
diag_stats_df <- bind_rows(all_diag_stats) # <--- NEW: Bind the list into a dataframe
...
write.csv(diag_stats_df, file.path(results_PATH, "bayes_reporting_diagnostics.csv"), row.names = FALSE) # <--- NEW: Save the CSV
...
#Final reloo for last two models 
```{r}
library(future)
