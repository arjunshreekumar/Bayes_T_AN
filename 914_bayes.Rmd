...
```{r}
# Word-ready baseline characteristics table WITH variable names
library(tidyverse)
library(tableone)
library(flextable)
library(officer)
...
# ---- SET treatment variable name (edit if needed) ----
...
run_bayesian_model <- function(data, rois, sessions_keep, file_out) {
  ...
  for (roi in rois) {
    ...
    if (nrow(model_dat) == 0) {
      warning("Skipping ROI '", roi, "' (3-factor) because no complete cases after NA filtering.")
      next
    }
...
run_bayes_meal_averaged <- function(data_wide, rois, sessions_keep, file_out) {
  ...
  for (roi in rois) {
    ...
    # Long format for this ROI across requested sessions & meals
    dat_long <- dat %>%
...
# ============================================================================
if (refit_models) {
  # This refits ALL ROIs for EACH model type
...
```{r}
# ============================================================
# SELECTIVE RELOO FOR THE 4 FLAGGED MODELS ONLY
...
# ============================================================
# RE-RUN ONLY THE FAILED RELOO FITS (2factor ones) SEQUENTIALLY
# ============================================================
