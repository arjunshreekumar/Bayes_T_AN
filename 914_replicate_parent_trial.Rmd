...
# We use tableone to calculate the raw numbers, but we turn OFF its p-values
t1 <- CreateTableOne(
  vars = vars,
  strata = ".treat",
  data = data2,
  ...
  includeNA = FALSE,
  addOverall = TRUE,
  test = FALSE # We will inject our own P-values
)
...
# ---- 4. Calculate Custom P-Values (CORRECTED) ----
...
p_values_cont <- map_dfr(contVars, function(v) {
  ...
  # Return row with P-value converted to CHARACTER to match tableone output
...
# Extract and ensure p column is character
...
final_df <- t1_df %>%
  # Join our custom p-values. 
  # Note: This join works perfectly for Continuous vars. 
  # For Categorical vars, tableone puts the p-value on the main row.
  left_join(p_values_all %>% select(rowname, p_formatted), by = "rowname") %>%
  mutate(
    # Use the labels mapping
    Characteristic = recode(rowname, !!!labels),
    # Fill in labels that didn't match (categorical levels usually)
...
# ---- 3. Run ANCOVA Loop (Dynamic Formula Method) ----
results_list <- list()
...
for (label in names(ancova_vars)) {
  ...
  # 5. Calculate Adjusted Means (Least Squares Means)
...
# ---- 3. Helper Function: Robust LMM with Fallback ----
run_lmm_analysis <- function(data, col_names, outcome_label) {
  ...
  # We use tryCatch to handle errors/warnings gracefully.
  
  result <- tryCatch({
    # ATTEMPT 1: Random Slope (Original Method) + Better Optimizer
    # 'bobyqa' is more robust for these convergence issues
    model <- lmer(Value ~ Time * Treatment_Factor + (1 + Time | Subject), 
                  data = long_df,
                  control = lmerControl(optimizer = "bobyqa"))
    
    # Check for Singularity (a common hidden failure)
